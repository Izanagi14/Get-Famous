...
class tFeatureExtractor:
    ...
    # Output file path
    HDF5_FILEPATH = "tfeatures.hdf5"

    # MongoDB database and table
    TWITTER_DATABASE = "m_project1"
    TWEETS_TABLE = "newc"
    ...
    def __getFeatures(tweet, ids, featuresList, viralityList, keepTweetWithoutHashtags):
        ...
        features.append(tFeatureExtractor.__twitterDateToDaysFromNow(user['created_at']))
        if user['verified']:
            features.append(1)
        else:
            features.append(0)
    ...
    def loadFromDB(limit=0, tweets_id=None, keepTweetWithoutHashtags=False):
        ...
        try:
            ...
            db = conn[tFeatureExtractor.TWITTER_DATABASE]
            collection = db[tFeatureExtractor.TWEETS_TABLE]
            if tweets_id is None:
                for tweet in collection.find(limit=limit):
                    tFeatureExtractor.__getFeatures(tweet, ids, featuresList, viralityList, keepTweetWithoutHashtags)
            else:
                for tweet_id in tweets_id:
                    tFeatureExtractor.__getFeatures(db[tFeatureExtractor.TWEETS_TABLE].find_one({"id": tweet_id}),
    ...
    def load(force=False, keepTweetWithoutHashtags=False):
        ...
        try:
            f = h5py.File(tFeatureExtractor.HDF5_FILEPATH, 'r')
            ids = f["IDs"]
            features = f["Features"]
            virality = f["Virality"]
            ...
            if force:
                ...
                ids, features, virality = tFeatureExtractor.loadFromDB(keepTweetWithoutHashtags=keepTweetWithoutHashtags)
                tFeatureExtractor.dump(ids, features, virality)
                print "> {} rows loaded".format(len(ids))
                return ids, features, virality
    ...
    def dump(ids, featuresList, viralityList):
        ...
        output = h5py.File(tFeatureExtractor.HDF5_FILEPATH, "w")
        ...
        col_name = 'Column '
        for idx, label in enumerate(tFeatureExtractor.FEATURE_LABEL):
            dset_features.attrs[col_name + str(idx)] = label

        # Virality
        dset_virality = output.create_dataset("Virality", data=(viralityList))
        dset_virality.attrs["Column 0"] = tFeatureExtractor.VIRALITY_LABEL[0]
...
if __name__ == "__main__":
    ...
    ids, features, virality = tFeatureExtractor.load(force=True)
    print(features)
